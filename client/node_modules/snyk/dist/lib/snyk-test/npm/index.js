...
function test(root, options) {
    ...
    return fs.exists(root)
        ...
        if (!exists) {
            var module_1 = moduleToObject(root);
            debug('testing remote: %s', module_1.name + '@' + module_1.version);
            payload.method = 'GET';
            payload.url += '/' +
                encodeURIComponent(module_1.name + '@' + module_1.version);
        ...
        return Promise.resolve()
            .then(function () {
            var isLockFilebased = (targetFile.endsWith('package-lock.json')
...
function generateDependenciesFromLockfile(root, options, targetFile) {
    var lockFileFullPath = path.resolve(root, targetFile);
    ...
    var lockFileType = targetFile.endsWith('yarn.lock') ?
    ...
    return spinner(resolveModuleSpinnerLabel)
        .then(function () {
        var strictOutOfSync = _.get(options, 'strictOutOfSync') !== 'false';
...
function getDependenciesFromNodeModules(root, options, targetFile) {
    var nodeModulesPath = path.join(path.dirname(path.resolve(root, targetFile)), 'node_modules');
    ...
    return spinner(lbl)
        ...
        return Promise.resolve().then(function () {
            ...
            return snyk.policy.loadFromText(res.policy)
                .then(function (policy) { return policy.filter(res, root); });
        }).then(function (res) {
