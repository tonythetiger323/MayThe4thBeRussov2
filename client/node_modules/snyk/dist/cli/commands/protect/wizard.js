...
function processPackageManager(options) {
    return __awaiter(this, void 0, void 0, function () {
        var packageManager, usupportedPackageManager;
        return __generator(this, function (_a) {
            packageManager = detect.detectPackageManager(cwd, options);
            usupportedPackageManager = unsupportedPackageManagers[packageManager];
            if (usupportedPackageManager) {
                return [2 /*return*/, Promise.reject("Snyk wizard for " + usupportedPackageManager + " projects is not currently supported")];
            }
            return [2 /*return*/, fs.exists(path.join('.', 'node_modules'))
...
function processWizardFlow(options) {
    spinner.sticky();
    var message = options['dry-run'] ? '*** dry run ****' : '~~~~ LIVE RUN ~~~~';
    debug(message);
    return snyk.policy.load(options['policy-path'], options)
        ...
        return auth.isAuthed().then(function (authed) {
            ...
            .then(function () { return authorization.actionAllowed('cliIgnore', options); })
                    ...
                    return snyk.policy.loadFromText(res.policy)
                        ...
                        return tryRequire(packageFile).then(function (pkg) {
                            ...
                            return interactive(res, pkg, combinedPolicy, options)
                                .then(function (answers) { return processAnswers(answers, cliPolicy, options); });
                        });
                    });
                });
            });
...
function processAnswers(answers, policy, options) {
    ...
    var isLockFileBased = targetFile.endsWith('package-lock.json') || targetFile.endsWith('yarn.lock');
    var pkg = {};
    ...
    var res = protect.generatePolicy(policy, tasks, live, options.packageManager)
        ...
        var addSnykToDependencies = answers['misc-add-test'] || answers['misc-add-protect'];
        var updateSnykFunc = function () { return protect.install(packageManager, ['snyk'], live); };
        if (addSnykToDependencies) {
            ...
            if (_.get(pkg, 'dependencies.snyk') ||
                ...
                var addSnykToProdDeps = answers['misc-add-protect'];
                var snykIsInDevDeps = _.get(pkg, 'devDependencies.snyk');
                if (addSnykToProdDeps) {
                    ...
                    updateSnykFunc = function () { return protect.installDev(packageManager, ['snyk'], live); };
                }
            }
        }
        if (addSnykToDependencies ||
