...
export class RsvpsController {
  ...
  async create(@Param() params, @Res() res: Response, @Body() rsvpData: Rsvp): Promise<any> {
    ...
    if (!(rsvpData && rsvpData.firstName && rsvpData.lastName && rsvpData.email && rsvpData.rsvp && rsvpData.guests && rsvpData.cars)) { return res.status(HttpStatus.FORBIDDEN).json({ message: 'RSVP, number of guests, and number of cars is required'});
    };
